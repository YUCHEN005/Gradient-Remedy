...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    #train_set=rats_Ach_waveform_8k_joint_enh_asr_train_10
    #valid_set=rats_Ach_waveform_8k_joint_enh_asr_valid_10
    ...
    # shellcheck disable=SC2086
    ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
        python3 -m espnet2.bin.gradient_remedy \
             --collect_stats true \
            --use_preprocessor true \
            --bpemodel "${bpemodel}" \
            --token_type "${token_type}" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} JOB=1:"${_nj}" "${_logdir}"/joint_inference.JOB.log \
             python3 -m espnet2.bin.gradient_remedy_inference \
                    --ngpu "${_ngpu}" \
                    --num_workers 0 \
                    ${_decode_data_param} \
