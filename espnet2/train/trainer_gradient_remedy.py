        ...
        #for iiter, (uttid, batch) in enumerate(
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    (loss_asr, loss_enh), stats, weight = model(**batch) # md add
                    #loss, stats, weight = model(uttid,**batch)
                stats = {k: v for k, v in stats.items() if v is not None}
                if ngpu > 1 or distributed:
                    # Apply weighted averaging for loss and stats
                    loss_asr = (loss_asr * weight.type(loss_asr.dtype)).sum()
                    loss_enh = (loss_enh * weight.type(loss_enh.dtype)).sum()
                ...
                if distributed:
                    ...
                    loss_asr *= torch.distributed.get_world_size()
                    loss_enh *= torch.distributed.get_world_size()
            ...
            for i, (key, params) in enumerate(model.named_parameters()):
                all_grads.append(params.grad.data if params.grad is not None else None)

            ### enhancement loss backward
            with reporter.measure_time("backward_time_enh"):
                optimizer.zero_grad(set_to_none=True)
                if scaler is not None:
                    scaler.scale(loss_enh).backward(retain_graph=True)
                else:
            ...
            for i, (key, params) in enumerate(model.named_parameters()):
                if key.startswith('enh_model'):
                    enh_grads.append(params.grad.data)

            ### asr loss backward
            with reporter.measure_time("backward_time_asr"):
                optimizer.zero_grad(set_to_none=True)
                if scaler is not None:
                    scaler.scale(loss_asr).backward()
                else:
                    loss_asr.backward()
            ...
            for i, (key, params) in enumerate(model.named_parameters()):
                if key.startswith('enh_model'):
                    ...
                    tan_theta = enh_grad.norm() / asr_grad.norm()

                    enh_asr_dot = torch.dot(enh_grad, asr_grad)
                    if enh_asr_dot < 0:
                    ...
                    if enh_mag / asr_mag > K and not cos_theta_prime == 0:
                        enh_grad = enh_grad * cos_theta_prime       # compress G_SE
                        asr_grad = asr_grad / cos_theta_prime       # stretch G_ASR

                    params.grad.data = (enh_grad + asr_grad).view_as(enh_grads[i]).to(params.device)

                if all_grads[i] is not None:
                    params.grad.data = params.grad.data + all_grads[i].to(params.device)
